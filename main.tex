...
To evaluate DeduceIt we deployed it in the winter 2013 offering of Coursera's \textit{Compilers} class, which assigned each student several DeduceIt problems in every week of the course. Out of the 7625 students who enrolled, 1788 of them participated actively, and 971 of these students completed at least one DeduceIt assignment. Overall system latency was $900ms$ on average.
...
Curiously, both rules come from the same assignment \textit{LL(1) Parsing} and dictate valid operations over a lookup table. However, while ``Nonterm Y input plus'' appears on a correct derivation path, ``Nonterm Y input end'' does not. Most students using the latter rule entered a valid but misleading conclusion on the previous derivation step; such a step is an ideal candidate for hint annotation on the assignment proof tree (e.g. a hint which suggests, ``while this step is correct, it doesn't lead to the solution.''). %Fixme: double check this stuff
...
\caption{We graph the system-wide cache hit rate against the number of student interactions with DeduceIt. The x-axis measures the total number of derivation steps entered into the system. The y-axis measures the cache hit rate as a percentage.}
...
We might address this issue in several ways. First, the backend prover can be made more efficient. Alternatively, we could distribute proof search across client-computers, passing a larger share of computation onto the students' machines.
