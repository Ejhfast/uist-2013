...
\author{
  \alignauthor Ethan Fast, Colleen Lee, Daphne Koller, Michael Bernstein, Alex Aiken\\
    ...
    \email{\{ethanfast, clee0, koller, aiken, msb\}@cs.stanford.edu}\\
}
...
Many rewrite languages are common enough that it makes sense for them to be shared among different assignments: for instance, languages which describe basic algebraic manipulation, predicate logic, or the expansion of regular expressions. DeduceIt allows instructors to reuse a rewrite language across assignments, once it has already been defined.

\subsubsection{The Assignment Proof Tree}

For each assignment, an instructor has real-time access to its proof tree. This tree tracks the history of all derivations associated with the assignment. These derivations share a common root node, and each step in a derivation maps to one of its children, the immediate parent of which is either the root node itself or else a node associated with the previously derived step. Every node maintains the details associated with its step as well as frequency information: a simple count of how many derivations have traversed it. The instructor may annotate any node in the tree with optional hints, and these will be visible to a student if she enters upon that path in her own derivation. An instructor may also override the behavior of the underlying derivation checker (e.g. forcibly label a given derivation step valid). This can be useful for nodes which make use of particularly long chains of free rules which can't be discovered and verified by proof search.
