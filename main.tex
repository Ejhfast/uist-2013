...
To verify the current state of a derivation, students must click on the button ``Update Proof." DeduceIt will respond with one of several kinds of feedback: if the derivation is valid so far, all steps will turn green; if it cannot parse the conclusion of some step in the derivation, that step will turn yellow; or if it can parse but not prove the conclusion of some step in the derivation, that step will turn red. Each valid step preceding an invalid step will remain green, and if an invalid step has a hint annotation on the assignment Proof Tree, an optional hint will adjacent to that step. If the conclusion of the latest step is equivalent to the goal expression, DeduceIt signals that the assignment is finished and does not prompt a new step in the derivation.
