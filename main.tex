...
\begin{abstract}
Large online courses have focused on topics and assignments that are gradable by simple checks such as multiple choice. However, in many domains, students may arrive at a large number of different valid solutions. We focus on \emph{derivations}: sequences of logical steps that are used commonly in assignments for technical, mathematical and scientific subjects. We present DeduceIt, a system for creating and grading derivation assignments across arbitrary formal domains with many valid solutions. DeduceIt supports formal reasoning for any set of rules and logical formalisms, and it allows instructors to specify the level of abstraction at which each assignment may be completed --- students may elide steps which are didactically unimportant to a derivation. The system is scalable and accessible to many thousands of students on the web. Rather than suffer from the computational cost of checking derivations from thousands of users simultaneously, DeduceIt benefits from the load: it introduces the idea of a \textit{proof cache}, a novel data structure which leverages a crowd of students to decrease the cost of checking derivations and providing real-time, constructive feedback. We evaluated DeduceIt with 1788 students in an online compilers course, finding that students take advantage of its incremental feedback and that instructors benefit from its structured insights into confusing course topics.
\end{abstract}
...
In this example we ask a student to show that two regular expressions are equal: $(A+B)* \equiv ((B+A).(B+A)*)+\epsilon$ using $+$, $*$, $\epsilon$, and $.$ as as union, kleen closure, epsilon, and concatenation. The student must derive the expression $((B+A).(B+A)*)+\epsilon$ starting from a set of givens, in this case the single expression $(A+B)*$, using several rules which are equality preserving transformations of regular expressions.
...
Since our student has been given only $(A+B)*$ from which to start her derivation, she reasons that she must select this expression from the givens input field on the first step. From the rule field she must select a rule from the list of assignment rules, and in the conclusion field she must enter a newly derived expression. She selects the ``Unfold" rule and tries it on the full starting expression, mentally computing:
$$((A + B).(A + B)*)+\epsilon$$
She enters this in the conclusion input field and submits the step. After a brief computation the derivation returns with her step highlighted in green: it was successful. A new set of input fields lies below her previous entry, querying her for the next line of the derivation.

\subsection{Skipping Didactically Unimportant Steps}

Our student now considers her previous conclusion: since it is nearly identical to the goal, she wants to use it for her next step. If she can transform its two sub-expressions $A+B$ to $B+A$, then she will have proven the goal and completed the assignment. She decides to apply Commutativity to each of the two $A+B$ sub-expressions, entering her result in the conclusion input box on the next line of the derivation. This new conclusion is identical to the goal expression: 
$$((B+A).(B+A)*)+\epsilon$$
...
These constraints eliminate the possibility of many simple mistakes, like mistypings, which we found surprisingly common in an earlier prototype of the system. The conclusion input field is unconstrained, however, and students can type into it any kind of expression they wish.
...
We show that students are successfully engaging with DeduceIt. Out of the X students who used DeduceIt in our online Compilers class, we observe an overall assignment completion rate of 98\% and an average time to completion of Y minutes over 10 assignments. Further, we find using a proof cache can increase the performance of the DeduceIt theorem prover by 76\%, enabling a much more responsive system. 

This paper also outlines how instructors can use DeduceIt to discover future course improvements. By analyzing assignment data (using structures like DeduceIt's proof tree), instructors can determine which parts of an assignment are most difficult for students to complete, or which course concepts are least well understood. Systems like DeduceIt strengthen the feedback loop between a class and its instructor.
